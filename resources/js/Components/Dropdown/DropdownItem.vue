...
const copy = () => {
  Clipboard.setSuccessMessage(
    props.copySuccessMessage ? props.copySuccessMessage : $t('system.copied_clipboard')
  )
    .setErrorMessage(
      props.copyErrorMessage ? props.copyErrorMessage : $t('system.failed_copied_clipboard')
    )
    .copy(props.copyText)
}
</script>
<template>
  <button
    v-if="as === 'button'"
    type="button"
    :class="[classes, sizeClass, 'outline-hidden focus:outline-hidden']"
    @click="copyable ? copy() : undefined"
  >
    <span v-if="$slots.icon && !copyable" :class="iconClass">
      <slot name="icon" />
    </span>
    <span v-if="copyable" :class="iconClass">
      <ClipboardIcon />
    </span>
    <slot />
  </button>
  ...
  <div
    v-else-if="as === 'div'"
    :class="[classes, sizeClass, copyable ? 'cursor-pointer' : '']"
    @click="copyable ? copy() : undefined"
  >
    <span v-if="$slots.icon && !copyable" :class="iconClass">
      <slot name="icon" />
