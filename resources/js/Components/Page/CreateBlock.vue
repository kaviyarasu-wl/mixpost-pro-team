...
<template>
  <Dropdown placement="bottom">
    ...
    <template #content>
      <DropdownItem as="button" @click="open('Editor')"> Editor </DropdownItem>
      <DropdownItem as="button" @click="open('Html')"> Html </DropdownItem>
    </template>
  </Dropdown>
  ...
  >
    <template #header>
      <span
        >{{ $t('page.create_block') }} [<span class="font-semibold capitalize">{{
          form.module
        }}</span
        >]</span
      >
    ...
    <template #body>
      <div class="mt-xs">
        <template v-if="modal">
          <FormBlock :form="form" :module-component="moduleComponent" />
        </template>
      </div>
    </template>

    <template #footer>
      <SecondaryButton class="mr-xs" @click="attemptClose">{{
        $t('general.cancel')
      }}</SecondaryButton>
