...
const deleteBlock = () => {
  confirmation()
    ...
    .onConfirm(dialog => {
      ...
      router.delete(
        route(`${routePrefix}.blocks.delete`, {
...
<template>
  <PureButton class="group-visible" @click="open">
    <template #icon>
      <PencilSquare />
    </template>
  </PureButton>
  ...
  >
    ...
    <template #footer>
      <SecondaryButton class="mr-xs rtl:mr-0 rtl:ml-xs" @click="attemptClose">{{
        $t('general.cancel')
      }}</SecondaryButton>
      <DangerButton class="mr-xs rtl:mr-0 rtl:ml-xs" @click="deleteBlock">{{
        $t('general.delete')
      }}</DangerButton>
