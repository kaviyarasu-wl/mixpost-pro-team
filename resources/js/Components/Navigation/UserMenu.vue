...
const openWorkspace = workspace => {
  ...
  router.post(
    route('mixpost.switchWorkspace', { workspace: workspace.uuid }),
    {},
    {
      onSuccess() {
...
<template>
  <Dropdown width-classes="w-64" placement="top-start">
    ...
    <template #content>
      <div
        v-if="workspaces.length || enterpriseConsole.multiple_workspace_enabled"
      ...
      >
        <template v-for="workspace in workspaces" :key="workspace.uuid">
          <div role="button" class="cursor-pointer group" @click="openWorkspace(workspace)">
            <div
            ...
            >
              <Avatar
                ...
                :background-color="workspace.hex_color"
                :name="workspace.name"
                size="md"
        ...
        <template
          v-if="
            enterpriseConsole.multiple_workspace_enabled ||
            (enterpriseConsole.url && !workspaces.some(workspace => workspace.owner_id === user.id))
          "
        >
          <a :href="enterpriseConsole.create_workspace_url" class="group">
            <div
              class="rounded-lg p-0.5 border-2 border-gray-200 group-hover:bg-gray-200 transition-colors ease-in-out duration-200"
            >
      ...
      <template
        v-if="
          !$page.props.is_admin_console &&
          isWorkspaceAdminRole &&
          enterpriseConsole.has_workspace_urls
        "
      >
        <MenuDelimiter />

        <DropdownItem
          v-if="enterpriseConsole.workspace_settings_url"
          :href="enterpriseConsole.workspace_settings_url"
          as="a"
        >
          <template #icon>
            <CogIcon />
        ...
        <DropdownItem
          v-if="enterpriseConsole.workspace_billing_url"
          :href="enterpriseConsole.workspace_billing_url"
          as="a"
        >
          <template #icon>
            <CreditCard />
      ...
      <DropdownItem
        v-if="user.is_admin && enterpriseConsole.url"
        :href="enterpriseConsole.url"
        as="a"
      >
        <template #icon>
          <Briefcase />
      ...
      <DropdownItem
        :href="!impersonating ? route('mixpost.logout') : enterpriseConsole.stop_impersonating_url"
        link-as="button"
        :link-method="!impersonating ? 'post' : 'delete'"
      >
        <template #icon>
