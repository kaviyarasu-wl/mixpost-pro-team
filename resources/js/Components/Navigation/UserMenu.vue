...
<template>
    <Dropdown width-classes="w-64" placement="top-start">
        <template #trigger>
            <div role="button" class="w-full flex justify-between">
                <div :class="{'hidden sm:flex' : responsive}" class="border flex items-center justify-between mr-4 px-4 rounded-md w-[calc(100%-3rem)]">
        ...
        <template #content>
            ...
            <DropdownItem v-if="workspaceCtx != undefined" :href="route('mixpost.profile.index', {workspace: workspaceCtx.id})">
                <template #icon>
                    <PencilSquareIcon/>
                </template>
            ...
            <DropdownItem v-if="workspaceCtx == undefined" :href="route('mixpost.profile.index.minimal')">
                <template #icon>
                    <PencilSquareIcon/>
                </template>
                {{ $t('profile.edit_profile') }}
            ...
            <!-- route('mixpost.logout') // Default Logout Method -->
            ...
            <template v-if="!impersonating">
                <a :href="route('mixpost.sso.logout')" class="flex items-center text-left w-full p-sm text-gray-800 hover:bg-primary-50 transition ease-in-out duration-200">
                    <span class="mr-xs rtl:mr-0 rtl:ml-xs">
                        <ArrowRightOnRectangleIcon/>
                    </span>
                    {{ $t('auth.sign_out') }}
                </a>
            ...
            <template v-if="impersonating">
                <DropdownItem :href="!impersonating ? route('mixpost.logout') : enterpriseConsole.stop_impersonating_url"
                          linkAs="button"
                          :linkMethod="!impersonating ? 'get' : 'delete'">
                    <template #icon>
                        <ArrowRightOnRectangleIcon/>
                    </template>
                    {{ $t('auth.sign_out') }}
