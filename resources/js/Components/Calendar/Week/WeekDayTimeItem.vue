...
const isDisabled = computed(() => {
  const cellDateTimeMinute = addMinutes(
    parseISO(`${props.dateSlot} ${props.timeSlot}`),
    props.minuteSlot['end']
  )
...
const add = () => {
  ...
  router.visit(
    route('mixpost.posts.create', { workspace: workspaceCtx.id, schedule_at: scheduleAt })
  )
}
</script>
<template>
  <div class="relative min-h-[50px] group" :style="style">
    <div
