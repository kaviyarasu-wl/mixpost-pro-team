  ...
  <div class="bg-white">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between py-lg row-px">
      <div class="flex items-center space-x-xs mb-xs md:mb-0">
        ...
        />
        <DateIndicator :selected-date="selectedDate" :week-starts-on="weekStartsOn" />
      </div>
    ...
    <div
      class="calendar-week-height-sm md:calendar-week-height-md xl:calendar-week-height overflow-y-auto"
      @scroll="onScroll"
    >
      <Weekdays
        :time-zone="timeZone"
      ...
      <div class="w-full grid grid-cols-week-time-sm md:grid-cols-week-time">
        <template v-for="(time, indexTime) in dayTimes" :key="indexTime">
          <template v-for="(minuteSlot, minuteSlotIndex) in minuteSlots" :key="minuteSlotIndex">
            <div class="text-center text-gray-400 text-sm font-semibold">
              {{ minuteSlotIndex === 0 ? time[timeFormat] : '' }}
            </div>

            <div
              v-for="(weekday, indexDay) in weekDays"
              :key="indexDay"
