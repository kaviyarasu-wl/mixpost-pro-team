...
<template>
  <div
    class="w-full relative flex rounded-md overflow-hidden border border-gray-200 hover:border-primary-500 transition-colors ease-in-out duration-200"
    role="button"
    aria-pressed="false"
    tabindex="0"
    @click="openPreview"
  >
    <div v-if="item.tags.length" class="flex flex-col h-full">
      <div
        ...
        :key="tag.id"
        class="w-sm h-full first:rounded-tl-md last:rounded-bl-md"
    ...
    <div class="w-full h-full p-1 md:p-sm bg-white">
      ...
      <div class="flex items-center justify-between mt-xs">
        ...
        <PostStatus :value="item.status" :show-name="false" class="hidden md:block" />
      </div>
    </div>
  </div>
