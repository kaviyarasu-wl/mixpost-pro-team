...
const selectPrevious = () => {
  const newSelectedDate = startOfMonth(subMonths(props.selectedDate, 1))
...
const selectNext = () => {
  const newSelectedDate = startOfMonth(addMonths(props.selectedDate, 1))
  emit('dateSelected', newSelectedDate)
}
</script>
<template>
  <div class="flex items-center">
    <SecondaryButton class="mr-xs" @click="selectCurrent">
      {{ $t('calendar.today') }}</SecondaryButton
    >

    <div class="flex items-center">
      <PureButton class="mr-xs" @click="selectPrevious">
        <ChevronLeftIcon />
      </PureButton>
