...
<template>
  <div>
    <EditorButton v-tooltip="$t('template.open_template_manager')" @click="openManager">
      <RectangleGroup />
    </EditorButton>
    ...
    >
      ...
      <template #footer>
        <template v-if="show === 'list'">
          <PrimaryButton class="mr-xs" @click="() => showForm(null)">
            {{ $t('template.save_as_template') }}
          </PrimaryButton>
        </template>

        <template v-if="show === 'form'">
          <SecondaryButton class="mr-xs" @click="showList">{{
            $t('general.go_back')
          }}</SecondaryButton>
        </template>
