...
const fetch = () => {
  ...
  axios
    .get(route('mixpost.templates.api.index', { workspace: workspaceCtx.id }))
    .then(response => {
...
const deleteTemplate = template => {
  axios.delete(
    route('mixpost.templates.api.delete', {
      ...
      <Masonry :items="templates" :columns="3">
        <template #default="{ item }">
          <ListGroup class="group">
            <ListItem :with-classes-for-last="false">
                ...
                <template #action>
                  <div class="flex justify-end">
                    ...
                    <Dropdown placement="bottom-end">
                      ...
                      <template #content>
                        <DropdownItem as="button" @click="$emit('edit', item)">
                          <PencilSquare class="w-5! h-5! mr-1" />
                          {{ $t('general.edit') }}
                        </DropdownItem>

                        <DropdownItem as="button" @click="deleteTemplate(item)">
