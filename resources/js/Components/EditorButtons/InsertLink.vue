...
<template>
  ...
  <DialogModal
    ...
    @close="close"
  >
    <template #header> {{ $t('general.insert') }}/{{ $t('editor.edit_link') }} </template>
    ...
    <template #body>
      <VerticalGroup>
        ...
        <div class="w-full flex">
          <Input id="href" v-model="href" type="text" class="mr-xs" />
          <SecondaryButton size="md" :disabled="!editor.isActive('link')" @click="remove">
            <template #icon>
              <Unlink />
            </template>
          </SecondaryButton>
