...
const handleImageLoad = () => {
  ...
  const orientation =
    firstImageRef.value.naturalHeight > firstImageRef.value.naturalWidth ? 'VERTICAL' : 'HORIZONTAL'
  ...
  <template v-if="firstImageOrientation === 'HORIZONTAL' || media.length > 4">
    <div class="flex flex-wrap gap-1">
      ...
      <div class="w-full flex grow-0 gap-1">
        <Box v-for="(item, index) in media.slice(media.length < 5 ? 1 : 2, 5)" :key="item.id">
          <AspectRatioBox>
            <CoverStyle :src="item.thumb_url" />
          </AspectRatioBox>
          <div
            v-if="index === 2 && media.length > 5"
            class="absolute top-0 right-0 bottom-0 left-0 flex items-center justify-center bg-black bg-opacity-60"
          >
  ...
  <template v-else>
    <AspectRatioBox>
      <div class="flex flex-row gap-1">
        <div
          class="flex grow-0"
          :class="{ 'w-1/2': media.length === 3, 'w-2/3': media.length > 3 }"
        >
          <Box>
        ...
        <div class="block" :class="{ 'w-1/2': media.length === 3, 'w-1/3': media.length > 3 }">
          <div class="flex flex-wrap gap-1">
            <Box v-for="item in media.slice(1)" :key="item.id">
              <AspectRatioBox>
                <CoverStyle :src="item.thumb_url" />
              </AspectRatioBox>
            </Box>
