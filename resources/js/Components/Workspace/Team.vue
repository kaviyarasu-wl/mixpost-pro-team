...
<template>
  <Panel>
    ...
    <template #action>
      <SecondaryButton size="md" @click="storeUserModel = true">
      ...
      <template #body>
        <template v-for="user in workspace.users" :key="user.id">
          <TableRow :hoverable="true">
            ...
            <TableCell>
              <Badge
                ><span class="capitalize">{{ user.role }}</span></Badge
              >
            </TableCell>
            <TableCell>
              <template v-if="user.id !== authUser.id">
                <Dropdown placement="bottom-end">
                  ...
                  <template #content>
                    <DropdownItem as="button" @click="openEditUserModal(user)">
                      <PencilSquare class="w-5! h-5! mr-1" />
                      {{ $t('team.edit_role') }}
                    </DropdownItem>

                    <DropdownItem as="button" @click="deleteUser(user.id)">
  ...
  <DialogModal :show="storeUserModel" max-width="md" @close="storeUserModel = false">
    ...
    <template #body>
      <div class="mt-sm">
        ...
        <Error
          v-if="formStoreUser.errors.user_id"
          :message="formStoreUser.errors.user_id"
          class="mt-xs"
        />
    ...
    <template #footer>
      <SecondaryButton class="mr-xs" @click="storeUserModel = false">{{
        $t('general.cancel')
      }}</SecondaryButton>
      <PrimaryButton
        :disabled="formStoreUser.processing || !$page.props.users.length"
        @click="storeUser"
      >
        {{ $t('general.add') }}
      </PrimaryButton>
  ...
  <DialogModal :show="editUserModal" max-width="md" @close="editUserModal = false">
    ...
    <template #footer>
      <SecondaryButton class="mr-xs" @click="editUserModal = false">{{
        $t('general.cancel')
      }}</SecondaryButton>
      <PrimaryButton :disabled="formEditUser.processing" @click="updateUserRole">
        {{ $t('team.change_role') }}
      </PrimaryButton>
    </template>
  </DialogModal>
