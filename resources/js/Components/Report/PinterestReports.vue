...
const getMetricCount = value => {
  return Object.prototype.hasOwnProperty.call(props.data.metrics, value)
    ? props.data.metrics[value]
    : 0
}
...
<template>
  <div class="row-px mt-2xl">
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-sm">
      <Panel>
        <template #title
          ><span v-tooltip="$t('service.pinterest.report.number_pins_saved')">{{
            $t('general.save')
          }}</span>
        </template>
      ...
      <Panel>
        <template #title
          ><span v-tooltip="$t('service.pinterest.report.number_pin_clicks')">{{
            $t('service.pinterest.report.pin_clicks')
          }}</span>
        </template>
      ...
      <Panel>
        <template #title
          ><span v-tooltip="$t('service.pinterest.report.number_impressions')">{{
      ...
      <Panel>
        <template #title
          ><span v-tooltip="$t('service.pinterest.report.number_outbound_clicks')">{{
            $t('service.pinterest.report.outbound_clicks')
          }}</span>
        </template>
        <div class="font-bold text-primary-500 text-2xl">
          {{ getMetricCount('outbound_click') }}
        </div>
      </Panel>
    </div>
  </div>
