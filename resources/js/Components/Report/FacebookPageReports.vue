...
<template>
  <div class="row-px mt-2xl">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-sm">
      ...
      <Panel>
        <template #title
          ><span v-tooltip="$t('service.facebook.report.number_times_post_engagements')">{{
            $t('service.facebook.report.post_engagements')
          }}</span>
        </template>
        <div class="font-bold text-primary-500 text-2xl">
          {{ getMetricCount('page_post_engagements') }}
        </div>
      </Panel>

      <Panel>
        <template #title
          ><span v-tooltip="$t('service.facebook.report.number_times_posts_impressions')">{{
            $t('service.facebook.report.posts_impressions')
          }}</span>
        </template>
