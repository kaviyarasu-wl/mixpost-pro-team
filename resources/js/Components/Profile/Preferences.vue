    ...
    <HorizontalGroup>
      ...
      <div>
        <Select v-model="form.locale">
          <option v-for="locale in locales" :key="locale.long" :value="locale.long">
        ...
        <span v-if="isLocaleInBeta(form.locale)" class="text-xs text-gray-500 mt-xs block">
          🚧 Language <span class="font-medium">{{ form.locale }}</span> is in beta and may not be
          fully translated or can contain errors.
        </span>
      </div>
    </HorizontalGroup>
    ...
    <HorizontalGroup class="mt-lg">
      ...
      <div>
        <Select v-model="form.timezone">
          <optgroup v-for="(list, groupName) in timezoneList" :key="groupName" :label="groupName">
            <option
