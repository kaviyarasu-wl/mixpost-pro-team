...
<template>
  <div class="w-full h-full flex flex-col pb-2xl pt-20 bg-white border-r border-gray-200">
    ...
    <div class="flex flex-col space-y-lg overflow-y-auto px-xl h-full">
      <MenuGroupBody>
        <MenuItem
          :url="route('mixpost.dashboardAdmin')"
        ...
        <template v-if="!enterpriseConsole.url">
          <MenuItem
            :url="route('mixpost.users.index')"
            :active="$page.component === 'Admin/Users/Users'"
          >
            <template #icon>
              <UsersIcon />
            </template>
          ...
          <MenuItem
            :url="route('mixpost.workspaces.index')"
      ...
      <MenuGroupBody>
        <MenuItem
          :url="route('mixpost.services.index')"
        ...
        <MenuItem
          :url="route('mixpost.configs.general.form')"
        ...
        <MenuItem
          :url="route('mixpost.pages.index')"
          :active="$page.component === 'Admin/Pages/Pages'"
        >
          <template #icon>
            <BookOpen />
          </template>
      ...
      <MenuGroupBody>
        <template v-if="enterpriseConsole.is_system_webhook_enabled">
          <MenuItem
            :url="route('mixpost.system.webhooks.index')"
        ...
        <MenuItem
          :url="`/${$page.props.app.horizon_path}/dashboard`"
          :external="true"
        ...
        <MenuItem
          :url="route('mixpost.system.status')"
          :active="$page.component === 'Admin/System/Status'"
        >
          <template #icon>
            <InformationCircle />
          </template>
        ...
        <MenuItem
          :url="route('mixpost.system.logs.index')"
          :active="$page.component === 'Admin/System/Logs'"
        >
          <template #icon>
            <Document />
          </template>
