...
import LightButtonLink from "@/Components/Button/LightButtonLink.vue"
...
<template>
    <div class="w-full h-full flex flex-col py-lg bg-white border-r border-gray-200">
        <div :class="{'': isWorkspaceEditorRole}" class="relative px-sm flex items-center flex-row">
            <Link :href="route('mixpost.dashboard', {workspace: workspaceCtx.id})">
                <Logo/>
            </Link>
            <a :href="gwUrl('dashboard')"
                target="_blank"
                class="bg-gray-100 duration-200 ease-in-out focus:shadow-outline-indigo text-xs font-medium hover:bg-gray-200 inline-flex items-center px-2 py-2 rounded-lg transition ml-1">
                <ArrowLeft width="4" height="4" />
                <span class="pl-xs">Back</span>
            </a>
        </div>
        ...
        <div v-if="isWorkspaceEditorRole" class="flex px-xl">
            <LightButtonLink :href="route('mixpost.posts.create', {workspace: workspaceCtx.id})" class="w-full">
                <template #icon>
                    <PlusIcon/>
                </template>
                {{ $t("media.create_post") }}
        ...
        <div class="flex flex-col space-y-lg overflow-y-auto px-xl mt-5 h-full">
            <MenuGroupBody>
            ...
            <MenuDelimiter/>
            <MenuGroupHeader :create-url="!isWorkspaceEditorRole ? null : route('mixpost.posts.create', {workspace: workspaceCtx.id})" tooltipText="Create Post">
                {{ $t('post.content') }}
                <template #icon>
                    <PlusIcon/>
                </template>
