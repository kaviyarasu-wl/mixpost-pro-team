...
<template>
  <div class="w-full h-full flex flex-col py-2xl bg-white border-r border-gray-200">
    ...
    <div v-if="isWorkspaceEditorRole" class="flex px-xl">
      <DarkButtonLink
        :href="route('mixpost.posts.create', { workspace: workspaceCtx.id })"
        class="w-full"
      >
        <template #icon>
          <PlusIcon />
    ...
    <div class="flex flex-col space-y-lg overflow-y-auto px-xl mt-2xl h-full">
      <MenuGroupBody>
        <MenuItem
          :url="route('mixpost.dashboard', { workspace: workspaceCtx.id })"
      ...
      <MenuGroupHeader
        :create-url="
          !isWorkspaceEditorRole
            ? null
            : route('mixpost.posts.create', { workspace: workspaceCtx.id })
        "
      >
        {{ $t('post.content') }}
        <template #icon>
      ...
      <MenuGroupBody>
        <MenuItem
          :url="route('mixpost.posts.index', { workspace: workspaceCtx.id })"
        ...
        <MenuItem
          :url="route('mixpost.calendar', { workspace: workspaceCtx.id })"
        ...
        <MenuItem
          :url="route('mixpost.media.index', { workspace: workspaceCtx.id })"
        ...
        <template v-if="isWorkspaceEditorRole">
          <MenuItem
            :url="route('mixpost.templates.index', { workspace: workspaceCtx.id })"
      ...
      <template v-if="isWorkspaceAdminRole">
        <MenuDelimiter />
        <MenuGroupHeader
          :create-url="route('mixpost.posts.create', { workspace: workspaceCtx.id })"
        >
          {{ $t('post.configuration') }}
        </MenuGroupHeader>
        <MenuGroupBody>
          <MenuItem
            :url="route('mixpost.accounts.index', { workspace: workspaceCtx.id })"
          ...
          <MenuItem
            :url="route('mixpost.postingSchedule.index', { workspace: workspaceCtx.id })"
          ...
          <MenuItem
            :url="route('mixpost.webhooks.index', { workspace: workspaceCtx.id })"
