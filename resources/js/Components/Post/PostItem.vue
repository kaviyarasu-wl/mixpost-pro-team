...
const content = computed(() => {
  ...
  const accountVersions = accounts.map(account => {
    ...
    return accountVersion
      ...
      : getOriginalVersion(props.item.versions).content[0]
  })
  ...
  <TableRow :hoverable="true">
    ...
    <TableCell :clickable="true" @click="openPreview">
      <div class="w-44">
        ...
        <div
          v-if="item.status === 'scheduled' || item.status === 'needs_approval'"
          class="text-sm mt-xs text-gray-500"
        >
    ...
    <TableCell :clickable="true" class="pl-0!" @click="openPreview">
    ...
    <TableCell>
      <div class="flex gap-xs">
        <div
          v-for="(account, index) in item.accounts.slice(0, 3)"
          :key="account.id"
          :class="{ '-ml-6': index > 0 }"
        >
          <Account
        ...
        <Dropdown v-if="item.accounts.length > 3" width-classes="w-64" placement="bottom-end">
          ...
          <template #content>
            <VerticallyScrollableContent>
              <template v-for="account in item.accounts.slice(3)" :key="account.id">
                <DropdownItem as="div">
                  <span class="mr-xs">
                    <Account
                      :provider="account.provider"
    ...
    <DialogModal :show="preview" :scrollable-body="true" @close="closePreview">
      <template #body>
