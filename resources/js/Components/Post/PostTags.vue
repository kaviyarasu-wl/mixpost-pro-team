...
<template>
    <div>
        <div class="flex items-center rtl:ml-xs">
            ...
            <SecondaryButton v-if="editAllowed" @click="openManager" :hiddenTextOnSmallScreen="true" size="md" class="border-primary-800 text-primary-500">
                <template #icon>
                    <TagIcon/>
                </template>
                {{ $t("post.labels") }}
        ...
        <DialogModal :show="showManager"
            ...
            <template #footer>
                ...
                <PrimaryButton @click="store" class="mr-xs"> {{ $t("general.save") }}</PrimaryButton>
            </template>
        </DialogModal>
    </div>
</template>
