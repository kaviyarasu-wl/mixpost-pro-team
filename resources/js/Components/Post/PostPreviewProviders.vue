...
const previews = computed(() => {
  return props.accounts
    .map(account => {
      ...
      if (
        accountVersion &&
        Object.prototype.hasOwnProperty.call(accountVersion.options, account.provider)
      ) {
        options = accountVersion.options[account.provider]
      } else {
        if (Object.prototype.hasOwnProperty.call(defaultVersion.value.options, account.provider)) {
  ...
  <div>
    <template v-if="accounts.length">
      <template v-for="preview in previews" :key="preview.account.id">
        <Alert
          v-if="hasErrors(preview.account.errors)"
          variant="error"
          :closeable="false"
          class="mb-1"
        >
        ...
        <div class="mb-lg last:mb-0 relative">
          ...
          <div class="absolute right-0 top-0 -mt-sm -mr-xs">
            <div class="flex items-center">
              <div
                v-if="preview.account.external_url"
              ...
              <div class="flex items-center justify-center p-md w-8 h-8 rounded-full bg-white">
                <div>
    ...
    <template v-else>
      ...
      <Panel class="mt-lg">
        <div class="flex items-start">
          <div class="mr-sm">
            <span
              class="inline-flex justify-center items-center shrink-0 w-10 h-10 rounded-full bg-gray-100"
            ></span>
          </div>
          <div class="w-full">
            <div class="flex items-center">
