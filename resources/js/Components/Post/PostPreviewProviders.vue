...
import PostErrorMessage from "./PostErrorMessage.vue";
...
<template>
    <div>
        <template v-if="accounts.length">
            <template v-for="preview in previews" :key="preview.account.id">
                <Alert v-if="hasErrors(preview.account.errors)"
                       ...
                       :closeable="false"
                       class="mb-1">
                    <PostErrorMessage :errors="preview.account.errors" />
                </Alert>
        ...
        <template v-else-if="props.item.status === 'published' || props.item.status === 'failed'">
            <div>
                <div>{{ $t('general.hi') }} 👋</div>
                <div>For deleted accounts, there is no preview available.</div>
            </div>
            <Panel class="mt-lg">
