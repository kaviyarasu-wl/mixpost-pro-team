...
const availableAccounts = computed(() => {
  return props.accounts.filter(account => {
    return (
      ...
      !props.versions.map(version => version.account_id).includes(account.id)
    )
  })
})
...
const versionsWithAccountData = computed(() => {
  const defaultVersion = {
    ...getOriginalVersion(props.versions),
    ...{ account: { name: $t('post.original') } }
  }

  const versionsBelongsToAccount = props.versions
    .map(version => {
    ...
    <div class="flex flex-wrap items-start">
      <Tabs class="mr-xs items-center">
        <template v-for="(version, index) in versionsWithAccountData" :key="version.account_id">
          <Tab
            :active="activeVersion === version.account_id"
            :tab-index="index"
            class="relative mb-xs group"
            @click="$emit('select', version.account_id)"
          >
            <ProviderIcon
            ...
            />
            <span
              v-if="version.is_original && namesOfAvailableAccounts"
              v-tooltip="namesOfAvailableAccounts"
              class="mr-xs"
              >{{ version.account.name }}</span
            >
            ...
            >
              <button
                ...
                @click.prevent.stop="confirmationRemoval = version"
              >
                >
                <XIcon class="w-4! h-4!" />
              </button>
            ...
            <template #content>
              <VerticallyScrollableContent max-height="xl">
                <template v-for="account in availableAccounts" :key="account.id">
                  <DropdownItem as="button" @click="$emit('add', account.id)">
                    <span class="mr-xs">
                      <Account
    ...
    >
      ...
      <template #body>
        <span v-if="confirmationRemoval">
          {{
            $t('post.confirmation_delete_version', {
              account_name: confirmationRemoval.account.name,
              account_provider: confirmationRemoval.account.provider
            })
          }}
        </span>
      </template>
      <template #footer>
        <SecondaryButton class="mr-xs" @click="closeConfirmationRemoval">
          {{ $t('general.cancel') }}
        </SecondaryButton>
