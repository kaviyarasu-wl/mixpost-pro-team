...
const activeVersionHasProvider = provider => {
  return selectedAccountsWithVersion.value.some(
    account => props.activeVersion === account.id && account.provider === provider
  )
}
