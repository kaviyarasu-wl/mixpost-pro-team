...
const save = () => {
  ...
  router.put(route('mixpost.services.update', { service: 'adobe_express' }), props.form, {
    preserveScroll: true,
    onSuccess() {
      notify('success', $t('service.service_saved', { service: 'Adobe Express' }))
    },
    onError: err => {
      errors.value = err
    }
...
<template>
  <Panel>
    ...
    <template #description>
      <ReadDocHelp
        :href="`${$page.props.mixpost.docs_link}/services/media/adobe-express`"
        class="mt-xs"
      />
    </template>
