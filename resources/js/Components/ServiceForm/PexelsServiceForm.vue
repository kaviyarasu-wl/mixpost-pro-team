...
const save = () => {
  ...
  router.put(route('mixpost.services.update', { service: 'pexels' }), props.form, {
    preserveScroll: true,
    onSuccess() {
      notify('success', $t('service.service_saved', { service: 'pexels' }))
    },
    onError: err => {
      errors.value = err
    }
...
<template>
  <Panel>
    ...
    <template #description>
      <p>{{ $t('service.stock_photo.can_use_external_photos', { provider: 'Pexels' }) }}</p>
      <p>
        <a href="https://www.pexels.com/api/key" class="link" target="_blank">{{
          $t('service.generate_api_key')
        }}</a
        >.
      </p>
      <ReadDocHelp :href="`${$page.props.mixpost.docs_link}/services/media/pexels`" class="mt-xs" />
    </template>
