...
import PrimaryButtonLink from "../../../Components/Button/PrimaryButtonLink.vue";
...
const emptyAccount = ref(props.accounts.length === 0);
</script>
<template>
    ...
    <div class="flex flex-col grow h-full overflow-y-auto">
                        ...
                        <PageHeader :title="$t('post.your_post')" class="m-container mx-auto">
                            <div v-if="$page.props.post" class="flex items-center">
                                ...
                                <div class="flex items-center" :class="{'ml-lg': triedToSave || hasError}">
                                    <div
                                        :class="{'hidden': !triedToSave, 'animate-ping': isLoading, 'bg-lime-600': !hasError, 'bg-red-500': hasError}"
        ...
        <AddAccountModal :show="emptyAccount" @close="emptyAccount = false">
            <template #header>
                Account Required
            </template>
            <template #body>
                You need to add an account before creating a post. Please add your account and try again.
            ...
            <template #footer>
                <SecondaryButton @click="emptyAccount = false" class="mr-xs">
                    {{ $t("general.cancel") }}
                </SecondaryButton>
                <PrimaryButtonLink :href="route('mixpost.accounts.index', {workspace: workspaceCtx.id})">
                    <template #icon>
                        <PlusIcon/>
                    </template>
                    Add Account
