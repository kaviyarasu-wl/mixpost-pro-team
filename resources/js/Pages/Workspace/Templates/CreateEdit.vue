...
const store = (data, redirectToCreateRoute = false) => {
    ...
    axios.post(route('mixpost.templates.api.store', {workspace: workspaceCtx.id}), data)
        .then((response) => {
            ...
            <form @submit.prevent="save">
                <Panel>
                    ...
                    <Flex :responsive="false" class="items-center mt-lg">
                        <LightButton :disabled="isLoading" type="submit" class="mr-xs text-black border border-[#dbeafe]" weight="normal">
                            {{ $t('template.save_template') }}
                        </LightButton>

                        <template v-if="!template">
                            <PrimaryButton @click="() => { store(getFormattedData(), true) }" :disabled="isLoading"
                                           type="button" :isUppercase="false">
