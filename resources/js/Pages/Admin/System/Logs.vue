    ...
    <div class="w-full row-px">
      <Tabs>
        <template v-for="log in logs" :key="log.name">
          <Tab :active="activeLog === log.name" @click="activeLog = log.name">
            <template #icon>
              <Document />
            </template>
    ...
    <div class="mt-lg row-px w-full">
      <Panel class="mt-lg">
        <template v-if="logs.length">
          ...
          <div class="mt-md">
            <a
              :href="route('mixpost.system.logs.download', { filename: activeLogItem.name })"
              target="_blank"
            >
              <PrimaryButton class="mr-xs rtl:mr-0 rtl:ml-xs">{{
                $t('general.download')
              }}</PrimaryButton>
            </a>
