    ...
    <form @submit.prevent="submit">
      <Panel>
        ...
        <template v-for="(error, key) in form.errors" :key="key">
          <Error :message="error" class="mb-xs" />
        </template>
        ...
        <template v-if="isUsingAppCode">
          <HorizontalGroup>
            ...
            <Input id="code" v-model="form.code" type="text" class="w-full" autofocus required />
          </HorizontalGroup>
        </template>
        ...
        <Flex class="justify-between mt-lg">
          ...
          <template v-if="isUsingAppCode">
            <div role="button" class="link-primary" @click="useRecoveryCode">
              {{ $t('auth.use_recovery_code') }}
            </div>
          </template>

          <template v-if="isUsingRecoveryCode">
            <div role="button" class="link-primary" @click="useAppCode">
