...
export default function usePostCharacterLimit(props) {
  ...
  const currentCharUsed = computed(() =>
    getTextLength(
      currentCharMaxLimit.value?.provider.id,
      getTextFromHtmlString(getPostVersionContentBody(props.activeVersion))
    )
  )
  const currentCharLeft = computed(() =>
    currentCharMaxLimit.value?.limit || null
      ? calculateCharLeft(currentCharMaxLimit.value?.limit, currentCharUsed.value)
      : null
  )
