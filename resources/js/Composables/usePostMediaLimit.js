...
export default function usePostMediaLimit(props) {
  ...
  const accountsWithoutVersion = computed(() =>
    props.selectedAccounts.filter(account => !accountHasVersion(props.versions, account.id))
  )
  ...
  const getMediaLimitForType = (boundary, mediaType, postType, account) => {
    ...
    if (Object.prototype.hasOwnProperty.call(rules, postType)) {
  ...
  const getLowestMaxMediaLimits = version => {
    ...
    const accounts =
      version === 0
        ? accountsWithoutVersion.value
        : props.selectedAccounts.filter(account => account.id === version)
    ...
    mediaTypesBasic.forEach(mediaType => {
      ...
      if (accountsLimit.length) {
        ...
        results[mediaType] = accountsLimit.reduce((max, current) => {
          if (
            current.priority > max.priority ||
            (current.priority === max.priority && current.limit > max.limit)
          ) {
            return current
          }
          return max
  ...
  const currentMedia = computed(
    () =>
      getEnabledVersions().find(version => version.account_id === props.activeVersion)?.content[
        props.activeContent
      ]?.media
  )
